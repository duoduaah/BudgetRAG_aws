...
def create_deployment_package(
    ...
    if requirements:
        print(f"   Installing dependencies for Lambda (Amazon Linux): {', '.join(requirements)}")
        
        # OLD METHOD (commented out - may have platform incompatibility issues)
        ...
        # NEW METHOD - Install each package with Lambda-compatible platform
        for req in requirements:
            print(f"      Installing {req}...")
            result = subprocess.run(
                f"pip install --platform manylinux2014_x86_64 "
                f"--implementation cp --python-version 3.10 "
                f"--only-binary=:all: --upgrade "
                f"--target {package_dir} {req}",
                shell=True,
                capture_output=True,
                text=True
            )
            if result.returncode != 0:
                print(f"      ⚠️ Warning: {req} may have failed to install")
